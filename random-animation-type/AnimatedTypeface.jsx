...
import './AnimatedTypeface.css'; // We'll define styles separately
...
const AnimatedCharacter = ({ 
  ...
  }, [delay, currentAnimation, onAnimationComplete]);
  
  const handleClick = () => {
    if (!interactive) return;
    
    // Reset and play random animation
  ...
  const variant = ANIMATION_VARIANTS[currentAnimation];
  const pathLength = getPathLength();
  
  return (
    <div 
      className="animated-char-container"
      onClick={handleClick}
      style={{
        width: size,
        height: size * 1.6,
      ...
      <svg 
        ...
        style={{
          ...
          transform: isAnimating ? 
            `scale(${variant.animate.scale || 1}) rotate(${variant.animate.rotate || 0}deg)` : 
            `scale(${variant.initial.scale || 1}) rotate(${variant.initial.rotate || 0}deg)`,
          opacity: isAnimating ? variant.animate.opacity : variant.initial.opacity,
          transition: `all ${variant.transition.duration}s ${variant.transition.ease || 'ease'}`
        }}
      >
        <path
          ref={pathRef}
          d={charPath}
          stroke={color}
          strokeWidth={isAnimating ? (variant.animate.strokeWidth || 3) : (variant.initial.strokeWidth || 3)}
          fill="none"
          strokeLinecap="round"
          strokeLinejoin="round"
          style={{
            strokeDasharray: currentAnimation === 'draw' || currentAnimation === 'morph' ? pathLength : 0,
            strokeDashoffset: currentAnimation === 'draw' || currentAnimation === 'morph' ? 
              (isAnimating ? 0 : pathLength) : 0,
            transition: `all ${variant.transition.duration}s ${variant.transition.ease || 'ease'}`
          }}
        />
      </svg>
